...
public class MemberController {
	...
	public String insertMember(Member m ,Model model,
		...
		// => 숫자 타입의 데이터(나이, age)가 값이 없을 경우 400 에러가 발생한다
		// => DB 데이터 처리 시 자동 형변환 되므로 정수형 -> 문자열 형변환하여 처리 
		
		// => 비밀번호 값을 입력된 갑 (평문) 저장하는 것이 아니라 
		// *(암호문) : 원래 값을 알아보기 어렵게 만든 값(암호문)으로 저장할 것임 
		// Spring-security 라이브러리 추가 (pom.xml 등록)
		//BcryptPasswordEncoder 클래스를 빈으로 등록(xml)
		// *추가한 파일을 서버 구동 시 pre-loading할 수 있도록 web.xml 파일에 설정 
		System.out.println("평문 -->"+  m.getUserPwd());
		System.out.println("암호문 -->" + bCrypt.encode(m.getUserPwd()));
		
		m.setUserPwd(bCrypt.encode(m.getUserPwd()));
		// 멤버 객체 비빌번호 평문을 암호문으로 변경
		
		int result = mService.insertMember(m);
		
		if(result > 0) {
			session.setAttribute("alertMsg", "강사님을 존경합니다");
			// 회원가입 성공 = > 성공메세지 + 메인페이지 url 재요청 
			return"redirect:/";
		}else {
			// 회원 가입 실패 => 에러메세지 + 에러페이지 포워딩
			model.addAttribute("errorMsg", "강사님을 존경하지 않습니다");
			// <ModelAndView insert(ModelAndView mv)
			//mv.addObject("errorMsg", "메세지")
			// View // 응답화면 관련 처리 
